...
class OpenRouterKeyManager {
    ...
    public reportError(errorMsg: string) {
        ...
        const status = this.keyStatuses.get(currentKey);
        if (status) {
            const isQuotaError = errorMsg.includes("429") || errorMsg.toLowerCase().includes("rate limit") || errorMsg.toLowerCase().includes("no credits") || errorMsg.toLowerCase().includes("insufficient credits") || errorMsg.toLowerCase().includes("too many requests");
            if (isQuotaError) {
