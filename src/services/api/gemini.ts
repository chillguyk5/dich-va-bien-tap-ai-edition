 ...
 * mà tự quản lý key/rate limit riêng (OpenRouter, DeepSeek). Coi cả hai như nhau trong
 * smartExecution: chọn tuần tự theo danh sách, lỗi thì loại khỏi danh sách thử (blacklist)
 * thay vì đánh dấu "hết quota" theo kiểu Gemini.
 */
const isSatelliteModel = (id: string) => id.startsWith('openrouter:') || id.startsWith('deepseek:');
...
export const smartExecution = async <T>(
    ...
    while (iterations++ < MAX_ITERATIONS) {
        ...
        try {
            ...
            // For satellite models (OpenRouter/DeepSeek), we handle failures directly, không đụng quotaManager Gemini
            if (isSatelliteModel(selectedId)) {
