        ...
        for (let i = 0; i < chunks.length; i += CONCURRENCY) {
            ...
            const batchPromises = batch.map(async (chunk, idx) => {
                ...
                try {
                    ...
                    try {
                        const flashRes = await analyzeContextBatch(chunk, storyInfo, dictionary, useSearch, ['gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3-flash-preview'], additionalRules + "\nLƯU Ý: ĐÂY LÀ BẢN PHÂN TÍCH THÔ DO HẾT QUOTA. CHỈ TRÍCH XUẤT NHANH CÁC DANH TỪ RIÊNG.", enabledModels);
