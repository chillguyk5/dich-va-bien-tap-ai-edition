...
// trước: Gemma 26B > Gemma 31B > 3.1 Flash Lite > 3.5 Flash Lite > 3.5 Flash.
...
export const validateBatchWithAI = async (
    ...
    // NEW: Nếu batch này vừa được dịch/cứu hộ bằng OpenRouter hoặc DeepSeek, hậu kiểm (Tier 2)
    ...
    if (useOpenRouter) {
        ...
        // Vệ tinh DeepSeek chỉ có đúng 1 model cố định vừa dịch thành công batch này.
        ...
        // Thứ tự ưu tiên hậu kiểm: Gemma 26B > Gemma 31B > 3.1 Flash Lite > 3.5 Flash Lite > 3.5 Flash.
        const targetModels = ['gemma-4-26b-a4b-it', 'gemma-4-31b-it', 'gemini-3.1-flash-lite', 'gemini-3.5-flash-lite', 'gemini-3.5-flash'];
    ...
    const rescueLabel = useOpenRouter ? ' qua OpenRouter (batch này được dịch bằng OpenRouter)' : useDeepSeek ? ' qua DeepSeek (batch này được dịch bằng DeepSeek)' : '';
        ...
        prompt += `Hãy trả về kết quả định dạng JSON chuẩn:
{
  "validations": {
    ...
    // vì lọt lưới. Đặt TRƯỚC Lượt 2 để các file bị điền bù này (nếu do vệ tinh OpenRouter/DeepSeek
    ...
    // isValid=false. Lý do: các model free/nhẹ trên OpenRouter (DeepSeek free, Gemma 26B free)
    ...
    if (useOpenRouter || useDeepSeek) {
        const rescueSourceLabel = useOpenRouter ? 'OpenRouter' : 'DeepSeek';
        const failedIds = Array.from(aiReport.entries()).filter(([, v]) => !v.isValid).map(([id]) => id);
        if (failedIds.length > 0) {
            const geminiCandidates = ['gemma-4-26b-a4b-it', 'gemma-4-31b-it', 'gemini-3.1-flash-lite', 'gemini-3.5-flash-lite', 'gemini-3.5-flash'].filter(m => enabledModels.includes(m) || enabledModels.length === 0);
