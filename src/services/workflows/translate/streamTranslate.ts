...
export const translateBatchStream = async (
    ...
    // Ưu tiên đọc thẳng "Bàn giao OpenRouter"/"Bàn giao DeepSeek" đã được useTranslator.ts gắn
    // sẵn vào errorMessage (tính theo retryCount qua getRescueTarget) - nếu không thấy tag rõ
    // ràng (ví dụ lỗi phát sinh ngay lần đầu, chưa qua useTranslator gắn tag), suy luận lại từ
    // fileRetryCount + key đang có, dùng đúng 1 helper getRescueTarget dùng chung cả 2 nơi.
    ...
    const taggedRescueTarget: 'openrouter' | 'deepseek' | null = files.some(f => f.errorMessage?.includes('Bàn giao DeepSeek')) ? 'deepseek'
        : files.some(f => f.errorMessage?.includes('Bàn giao OpenRouter')) ? 'openrouter'
        : null;
    const maxRetryFile = Math.max(0, ...files.map(f => f.fileRetryCount || 0));
    const inferredRescueTarget = taggedRescueTarget || getRescueTarget(maxRetryFile, hasOpenRouterKeyAvail, hasDeepSeekKeyAvail, 2);
    ...
    // "Bàn giao OpenRouter"/"Bàn giao DeepSeek" (taggedRescueTarget khác null — nghĩa là hệ thống
    // ĐÃ quyết định đây là ca cần cứu hộ ở lượt trước) nhưng chuỗi errorMessage đó lại không chứa
    // đúng các cụm "an toàn"/"safety"/"nghi vấn lỗi nội dung"/"lỗi kiểm định ai" (ví dụ do lỗi gốc
    // là "Thiếu kết quả từ API"/"Lỗi ngắt kết nối API" — vẫn được scheduler ở useTranslator.ts xếp
    // vào diện cứu hộ nhưng không khớp bộ từ khoá hẹp hơn ở đây), thì needsRescueFallback = false,
    // khiến khối override effectiveModels bên dưới bị BỎ QUA hoàn toàn dù inferredRescueTarget đã
    // tính đúng là 'deepseek'. Hậu quả thực tế: batch rơi lại về tier/model mặc định trước đó (có
    // thể là 'openrouter' từ lượt cứu hộ OpenRouter thất bại ngay trước), và vì nhánh `tier ===
    // 'openrouter'` phía trên KHÔNG kiểm tra openRouterKey có tồn tại hay không, nó vẫn cắm thẳng
    // model OpenRouter dù key rỗng — dẫn đến lỗi "Tất cả model đã thử đều gặp lỗi hoặc hết Quota"
    // ngay cả khi người dùng đã dán đúng DeepSeek Key. Sửa: coi taggedRescueTarget khác null cũng
    // là điều kiện đủ để kích hoạt needsRescueFallback, không chỉ dựa vào dò từ khoá nữa.
    ...
    if(onLog) onLog(`[DEBUG] hasStrictSafetyError=${hasStrictSafetyError}, hasValidationError=${hasValidationError}, rescueTarget=${inferredRescueTarget || 'none'}, openRouterKey.length=${openRouterKey ? openRouterKey.length : 0}, deepseekKey.length=${deepseekKey ? deepseekKey.length : 0}`);

    if (needsRescueFallback) {
        if (inferredRescueTarget === 'deepseek') {
        ...
        } else if (inferredRescueTarget === 'openrouter' && hasOpenRouterKeyAvail) {
        const fallBackModel = openRouterModel || 'google/gemma-4-26b-a4b-it:free';
        ...
        if (onLog) onLog(`⚠️ Phát hiện lỗi phức tạp (${hasStrictSafetyError ? 'Safety' : (hasValidationError ? 'Validation' : 'Đã gắn tag Bàn giao')}). Tự động dùng danh sách dự phòng Free OpenRouter: ${effectiveModels.join(', ')}...`);
        } else {
            if (hasStrictSafetyError) {
                throw new Error("BLOCKLIST: File bị chặn bởi Safety Filter và không có OpenRouter/DeepSeek API Key để vượt nghiệm.");
            } else {
                ...
                if (onLog) onLog(`⚠️ Hậu kiểm Tier 2 nghi vấn lỗi nội dung nhưng không có OpenRouter/DeepSeek API Key dự phòng. Bỏ qua chuyển hướng, thử dịch lại bằng model gốc (${effectiveModels.join(', ')}) — có thể Tier 2 chỉ báo nhầm.`);
            }
        }
    }
...
CRITICAL: DO NOT TRANSLATE THE TAGS. ALWAYS OUTPUT THE EXACT TAGS (e.g. ${startTag} and ${endTag}).`;
    ...
    return await smartExecution(effectiveModels, async mid => {
        ...
        // trước nên tra cứu đồng bộ - không cần gọi API như getOpenRouterModelInfo).
        ...
        try {
            ...
            if (mid.startsWith('openrouter:')) {
                ...
                // Nhánh dispatch DeepSeek - cố tình lặp lại y hệt logic parse thẻ part_N của
                // nhánh OpenRouter ở trên (thay vì tách hàm dùng chung) để tránh rủi ro phá vỡ
                // luồng OpenRouter đang chạy ổn định khi refactor.
        ...
        if (bad.length > 0) {
            ...
            const fixModels = (mid.startsWith('openrouter:') || mid.startsWith('deepseek:')) ? [mid] : enabledModels;
            const fixes = await performAggregatedRepair(bad, relDict, tier, globalContext, storyInfo, userPrompt, onLog, fixModels, undefined, shouldAbort, 'auto_fix', openRouterKey, deepseekKey);
