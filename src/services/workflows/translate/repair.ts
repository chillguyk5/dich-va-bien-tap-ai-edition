...
export const performAggregatedRepair = async (
    ...
    for (let i = 0; i < total; i += batchLimit) {
        ...
        try {
            const res = await repairBatch(chunk, chunkDict, tier, context, storyInfo, promptTemplate, onLog, enabledModels, shouldAbort, taskType, openRouterKey, deepseekKey);
            ...
            if (isSafetyError && (hasOR || hasDS)) {
                // Cứu hộ 2 tầng giống hệt useTranslator.ts: OpenRouter trước (nếu có) -> DeepSeek sau (nếu có).
                const rescueOrder: Array<{ target: 'openrouter' | 'deepseek', models: string[] }> = [];
                if (hasOR) rescueOrder.push({ target: 'openrouter', models: ['openrouter:google/gemma-4-26b-a4b-it:free'] });
                if (hasDS) rescueOrder.push({ target: 'deepseek', models: ['deepseek:deepseek-v4-flash'] });

                let rescued = false;
                for (const { target, models } of rescueOrder) {
                    const label = target === 'openrouter' ? 'OpenRouter' : 'DeepSeek';
                    if (onLog) onLog(`⚠️ Lỗi Safety Filter ở Batch ${batchNum}. Đang chuyển hướng vệ tinh qua ${label} để thử lại...`);
                    try {
                        const res = await repairBatch(chunk, chunkDict, tier, context, storyInfo, promptTemplate, onLog, models, shouldAbort, taskType, openRouterKey, deepseekKey);
...
export const repairTranslations = async (badLines: LineContext[], dictionary: string, tier: TranslationTier = 'normal', context?: string, storyInfo?: StoryInfo, promptTemplate?: string, onLog?: (msg: string) => void, enabledModels?: string[], openRouterKey?: string, deepseekKey?: string): Promise<{ index: number; text: string }[]> => {
    const map = await repairBatch(badLines.map(bl => ({ fileId: 'temp', lineIndex: bl.index, originalLine: bl.originalLine })), dictionary, tier, context, storyInfo, promptTemplate, onLog, enabledModels, undefined, 'smart_fix', openRouterKey, deepseekKey);
