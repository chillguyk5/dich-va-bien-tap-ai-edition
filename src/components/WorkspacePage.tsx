...
export const WorkspacePage: React.FC<WorkspacePageProps> = (props) => {
    ...
    const counts = useMemo(() => {
        return {
            ...
            mDeepSeek: props.files.filter(f => f.usedModel?.includes('deepseek:')).length,
            mManual: props.files.filter(f => f.usedModel?.includes('Thủ công')).length,
            mOther: props.files.filter(f => f.status === FileStatus.COMPLETED && (!f.usedModel || (!f.usedModel.includes('gemini-3.1-pro') && !f.usedModel.includes('gemini-3.7-flash') && !f.usedModel.includes('gemini-3.6-flash') && !f.usedModel.includes('gemini-3.5-flash') && !f.usedModel.includes('gemini-3.1-flash') && !f.usedModel.includes('gemini-3-flash') && !f.usedModel.includes('openrouter:') && !f.usedModel.includes('deepseek:') && !f.usedModel.includes('Thủ công')))).length,
        };
            ...
            filtered = props.files.filter(f => {
                ...
                if (props.filterModels.size > 0) { 
                    ...
                    modelMatch = ( 
                        ...
                        (props.filterModels.has('other') && f.status === FileStatus.COMPLETED && (!m || (!m.includes('gemini-3.1-pro') && !m.includes('gemini-3.7-flash') && !m.includes('gemini-3.6-flash') && !m.includes('gemini-3.5-flash') && !m.includes('gemini-3.1-flash') && !m.includes('gemini-3-flash') && !m.includes('openrouter:') && !m.includes('deepseek:') && !m.includes('Thủ công'))))
                    ); 
                }
