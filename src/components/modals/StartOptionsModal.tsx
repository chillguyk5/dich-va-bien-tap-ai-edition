            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                <div className="p-6 overflow-y-auto flex-1">
                    ...
                    <div className="space-y-3">
                        ...
                        <button onClick={() => setSelectedTier('deepseek')} className={`w-full p-4 rounded-2xl flex items-start gap-4 transition-all group text-left border ${selectedTier === 'deepseek' ? 'bg-teal-50 border-teal-300 ring-2 ring-teal-200 shadow-md' : 'bg-white border-slate-200 hover:border-teal-200'}`}>
                            <div className={`p-3 rounded-xl shadow-sm transition-colors ${selectedTier === 'deepseek' ? 'bg-teal-500 text-white' : 'bg-teal-50 text-teal-600 group-hover:bg-teal-100'}`}><Zap className="w-6 h-6" /></div>
                            <div><h4 className={`font-bold text-sm ${selectedTier === 'deepseek' ? 'text-teal-800' : 'text-slate-700'}`}>DeepSeek</h4><p className="text-xs text-slate-500">Dùng API DeepSeek. Chỉ khả dụng khi cấu hình API Key và chọn Model trong cài đặt DeepSeek.</p></div>
                        </button>
