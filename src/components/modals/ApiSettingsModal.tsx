...
export const ApiSettingsModal: React.FC<ApiSettingsModalProps> = ({ 
    ...
    const [activeTab, setActiveTab] = useState<'openrouter' | 'deepseek'>('openrouter');
    ...
    // --- DEEPSEEK: state riêng, cùng cấu trúc với OpenRouter ở trên ---
    ...
    }, [isOpen, deepseekKey, deepseekModel, updateDsStatuses]);
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-elevation-5 w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
                ...
                {/* TAB BAR: OpenRouter / DeepSeek */}
                <div className="flex items-center gap-6 px-6 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
                    <button
                        onClick={() => setActiveTab('openrouter')}
                        className={`py-3 text-sm font-bold border-b-2 transition-colors duration-200 ease-smooth ${activeTab === 'openrouter' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
                    >
                        OpenRouter
                    </button>
                    <button
                    ...
                    >
                        DeepSeek
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-teal-50 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400">DỰ PHÒNG</span>
                    </button>
                </div>
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="flex items-start gap-2">
                            <textarea
                                ...
                                className="flex-1 p-3 py-2 min-h-[70px] rounded-xl text-sm border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500 resize-y"
                                value={newDsKeyInput}
