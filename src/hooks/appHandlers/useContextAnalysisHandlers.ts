                    ...
                    if (activeEngine === 'deepseek') {
                        try {
                            if (config.mode === 'deep_context') {
                                return await analyzeContextBatch(chunk, config.updatedStoryInfo, core.additionalDictionary, false, undefined, effectiveAdditionalRules, core.enabledModels, 'deepseek', dsKey, dsModel);
                            } else {
                                return await analyzeNameBatch(chunk, config.updatedStoryInfo, config.mode as 'only_char' | 'full', false, effectiveAdditionalRules, undefined, core.enabledModels, 'deepseek', dsKey, dsModel);
                            }
                        } catch (e: any) {
