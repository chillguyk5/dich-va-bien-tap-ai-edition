...
// riêng" hoặc đã xác nhận cần "Bàn giao OpenRouter/DeepSeek") đều bị xếp chung xuống TẬN CUỐI
...
// OpenRouter/DeepSeek) trước khi hết Quota hoặc hết phiên làm việc - dù đã add đủ API Key.
...
export const useTranslator = (
    ...
    const processBatch = useCallback(async (batchIds: string[], tier: TranslationTier, myRunId: number) => {
        ...
        try {
            ...
            // "Bàn giao OpenRouter/DeepSeek", để chúng không bị chôn ở cuối hàng chờ dài.
            ...
            // đề (nếu có) không bao giờ kích hoạt được cơ chế cứu hộ (OpenRouter/DeepSeek), cứ dịch
                            ...
                            if ((resultsMap as any).streamError || specificErr.includes("Thiếu kết quả từ API") || specificErr.includes("Lỗi ngắt kết nối API")) {
                                ...
                                if (missingResultFiles.length === 1) { // First file missing
                                    ...
                                    if (tailScan?.isUnsafe) {
                                        ...
                                        const target = getRescueTarget(f.retryCount || 0, hasOR, hasDS, maxRetries);
                                        if (target) {
                                            const rescueBudget = getRescueBudget(hasOR, hasDS, maxRetries);
                                            ...
                                            const reason = (hasOR || hasDS) ? "Đã hết lượt cứu hộ OpenRouter/DeepSeek" : "Không có OpenRouter/DeepSeek dự phòng";
            ...
            if (shouldScanAndIsolate) {
                        ...
                        for (const f of batchFiles) {
                            ...
                            if (isUnsafe) {
                                ...
                                if (target) {
                                    ...
                                    ui.addLog(`🚨 Tệp ${f.name} bị Gemini chặn nội dung! Đánh dấu lỗi. (Mẹo: Thêm API Key OpenRouter/DeepSeek trong Cài đặt để dự phòng cho các tệp lỗi)`, "error");
                                }
                            } else {
                ...
                if (unsafeIds.size > 0) {
                    for (const uid of unsafeIds) {
                        ...
                        const target = getRescueTarget(uf?.retryCount || 0, rescueHasOR, rescueHasDS, rescueMaxRetries);
                ...
                if (unsafeIds.size > 0) {
                    ...
                    core.setFiles((prev: FileItem[]) => prev.map((item: FileItem) => {
                        if (unsafeIds.has(item.id)) {
                            const target = getRescueTarget(item.retryCount || 0, hasOR, hasDS, maxRetries);
                            if (target) {
                                const rescueBudget = getRescueBudget(hasOR, hasDS, maxRetries);
            ...
            core.setFiles((prev: FileItem[]) => prev.map((f: FileItem) => {
                if (batchIds.includes(f.id)) {
                    ...
                    // riêng (rescueBudget = tổng lượt của các vệ tinh đang có key) để OpenRouter/DeepSeek
                    // có đủ lượt cứu hộ theo đúng thứ tự OpenRouter trước -> DeepSeek sau.
                    if (isSafetyError) {
                        ...
                        if (batchIndex === 0) {
                            ...
                            if (target) {
                                ...
                                const reason = (hasOR || hasDS) ? 'đã hết lượt xử lý' : 'không có OpenRouter/DeepSeek dự phòng';
