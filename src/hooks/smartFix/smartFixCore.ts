...
export const useSmartFixCore = (core: any, ui: any, sharedState: any) => {
    ...
    const handleFixRemainingRaw = async (isSmartFixMode: boolean = false) => {
        ...
        try {
            ...
            const hasDeepSeekFile = allBadLines.some(bl => {
                const f = core.files.find((file: FileItem) => file.id === bl.fileId);
                return f?.usedModel?.startsWith('deepseek:');
            });
            ...
            if (hasOpenRouterFile && core.openRouterKey && core.openRouterModel) {
            ...
            } else if (hasDeepSeekFile && core.deepseekKey && core.deepseekModel) {
                ...
                const selectedDs = core.deepseekModel.split(',').map((s: string) => s.trim()).filter(Boolean);
                modelsForFix = (selectedDs.length > 0 ? selectedDs : ['deepseek-v4-flash']).map((m: string) => `deepseek:${m}`);
            }
